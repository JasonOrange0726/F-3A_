...
# This file lives under src/f3a/qwen3_235b_fp8_adapter.  Add src to
# sys.path so imports work no matter where the script is launched.
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from f3a.qwen3_235b_fp8_adapter.fp8_wrapper import DEFAULT_FP8_MODEL_PATH, build_fp8_model_from_args
...
def parse_main_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run F3A on Qwen3-VL-235B-A22B-Instruct-FP8 without touching existing code.")
    parser.add_argument("--model-path", default=DEFAULT_FP8_MODEL_PATH)
    parser.add_argument("--device", default="cuda:0")
    parser.add_argument("--torch-dtype", default="auto")
    parser.add_argument("--device-map", default="auto")
    parser.add_argument("--max-memory", default="", help="Optional Accelerate max_memory, e.g. '0:135GiB,1:135GiB,cpu:512GiB'.")
    ...
    parser.add_argument("--keep-ratios", nargs="+", type=float, default=[0.2])
    ...
    parser.add_argument("--skip-baseline", action="store_true", default=True, help="Default true for 235B; pass --run-baseline to enable full-token baseline.")
...
def make_eval_args(main_args: argparse.Namespace, dataset: str, keep_ratio: float, save_json: Path):
    ...
    argv = [
        ...
        "--device-map", main_args.device_map,
    ...
    args.max_memory = main_args.max_memory
    args.offload_folder = main_args.offload_folder
    args.attn_implementation = main_args.attn_implementation
    return args
...
def write_payload(save_path: Path, args, dataset, routed_summary, routed_results, baseline_summary=None, baseline_results=None):
    payload = {
        "config": vars(args),
        "adapter": "qwen3_235b_fp8_adapter",
...
def main() -> None:
    ...
    model_tag = Path(main_args.model_path.rstrip("/")).name.replace("models--Qwen--", "")

    first_args = make_eval_args(main_args, main_args.datasets[0], main_args.keep_ratios[0], out_dir / "_warmup.json")
    print(f"[load] fp8_model={main_args.model_path} device={main_args.device} device_map={main_args.device_map}", flush=True)
    model = build_fp8_model_from_args(first_args)
    ...
    for dataset_name in main_args.datasets:
        ...
        if main_args.skip_baseline:
            ...
            baseline_path = out_dir / f"{dataset_name}_{model_tag}_k100_baseline_fp8.json"
        ...
        for keep_ratio in main_args.keep_ratios:
            ...
            save_path = out_dir / f"{dataset_name}_{model_tag}_{suffix}_fp8.json"
            ...
            print(f"[run] dataset={dataset_name} config={suffix} routing={main_args.routing_mode}", flush=True)
    ...
    summary_path = out_dir / f"{model_tag}_fp8_ordered_summary.csv"
    fieldnames = [
        "dataset", "model", "config", "keep_ratio", "metric", "score", "baseline_score", "retention",
        "avg_keep_ratio_actual", "avg_selected_visual_tokens", "avg_full_visual_tokens", "json",
    ]
